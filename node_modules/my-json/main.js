...
function createClass(config, constructor, proto) {
	...
	var staticMethods = {
		...
		sqlFromSchema: function (schema, ordering) {
			...
			var orderBy = ordering && ordering.orderBy;
			if (typeof orderBy === 'string') {
				orderBy = [orderBy];
			}
			if (orderBy && Array.isArray(orderBy) && orderBy.length) {
				console.log(orderBy);
				var parts = orderBy.map(function (column) {
					var direction = column.split(':', 1)[0];
					if (direction === 'asc' || direction === 'desc') {
						column = column.substring(direction.length + 1);
						return config.columnForPath(column) + ' ' + direction.toUpperCase();
					} else {
						return config.columnForPath(column)
					}
				});
				result += '\n\tORDER BY ' + parts.join(', ');
				console.log(result);
